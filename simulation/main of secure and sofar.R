...
pare_secure <- function(sss){
  ...
  C = Ur%*%diag(Dr)%*%t(V)
  E_est = Y - X%*%C
  obj_sigmae <-  sim(p = q, n = n, delta = 2, thresholding = 'al', data = E_est, cov_true = Sigmae_real)
...
##sofar  setting of simulation example 2
pare_sofar <- function(sss){
  #set.seed(100+sss)
  n <- 200; p <- 25; q <- 15; nrank <- 3
  obj_Xsigma_real <- ar1.mat(0.3, p, 1:p)
  Xsigma_real <- obj_Xsigma_real$Sigma
  mydata <- rrr.simnew2(n, p, q, nrank, s2n = 1,
                        sigma = NULL, rho_X = 0.3, rho_E = 0.3, Xsigma = Xsigma_real, D = diag(c(200, 15, 5)))
  U0 = mydata$U
  D0 = mydata$D
  V0 = mydata$V
  dU0 = U0%*%D0
  X <- mydata$X; Y <- mydata$Y
  Sigmae_real <- mydata$sigmae
  est_rank <- STRS(Y, X)
  # est_rank
  # fit1 <- sofar(Y, X, ic.type = "BIC", nrank =  est_rank, 
  #               control = list(methodA = "adlasso", methodB = "adlasso",
...
##sofar for example 3
##sofar setting of simulation example 3
pare_sofar_nonsparse <- function(sss){
  #set.seed(100+sss)
  n <- 200; p <- 50; q <- 30; nrank <- 3
  obj_Xsigma_real <- ar1.mat(0.3, p, 1:p)
  Xsigma_real <- obj_Xsigma_real$Sigma
  mydata <- rrr.simnew3(n, p, q, nrank, s2n = 1,
                        sigma = NULL, rho_X = 0.3, rho_E = 0.3, Xsigma = Xsigma_real, D = diag(c(200, 15, 5)))
  U0 = mydata$U
  D0 = mydata$D
  V0 = mydata$V
