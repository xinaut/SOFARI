...
load(file = "D:/OneDrive/Rcode/SOFARI/jasa/real data/economic/realdata20231006.RData")
##datauu is the debiased estimate of matrix U
df = as.data.frame(cbind(as.matrix(1:dim(datauu)[1]), datauu)) #add an index colomn 1:456
...
p_1 +  geom_bar(
  ...
  stat="identity"
                     ...
                     text=element_text(size=12, family="serif"))  + labs(x="",y="",) + scale_x_discrete(name = "",labels = ""
...
#These variables are treated as very improtant 
#(the value 3 is determined by myself after looking at the plot)
